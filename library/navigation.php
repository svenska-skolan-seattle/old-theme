...
class theme_MenuWalker {
	...
	function display(&$output, $child_Ids) {
		...
		foreach($child_Ids as $child_Id){
			...
			if ($this->class == 'hmenu' && !$first && $this->level == 0) {
				$output .= "\t" .'<li class="hmenu-li-separator"><span class="hmenu-separator"> </span></li>' . "\n";
			}
			if ($this->class == 'vmenu') {
				if ($this->level == 0) {	
					if(!$first) {
					 	$output .= "\t" .'<li class="vmenu-separator"><span class="vmenu-separator-span"> </span></li>' . "\n";
					}
				} else {
					if(($this->level == 1) && $first) {
						$output .= "\t" .'<li class="vmenu-separator vmenu-separator-first"><span class="vmenu-separator-span"> </span></li>' . "\n";
					} else {
						$output .= "\t" .'<li class="vsubmenu-separator"><span class="vsubmenu-separator-span"> </span></li>' . "\n";
					}
				}	
			}
